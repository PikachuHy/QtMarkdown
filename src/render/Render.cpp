...
class RenderPrivate
 ...
 public:
  explicit RenderPrivate(DocPtr doc) : m_doc(doc) {
  ...
  void visit(InlineCode *node) override {
    ...
    save();
    setFont(codeFont());
    auto row = m_instructionGroup.size();
    auto col = m_instructionGroup[row - 1].size();
    if (currentLineCanDrawText(node->code()->toString(m_doc))) {
      node->code()->accept(this);
      auto rect = m_instructionGroup[row - 1][col]->config().rect;
      m_config.rect = QRect(rect.x() - 5, rect.y() - 5, rect.width() + 10, rect.height() + 10);
      m_config.brush = QBrush(QColor(249, 249, 249));
      auto &item = m_instructionGroup[row - 1];
      item.insert(item.begin() + col, new FillRectInstruction(m_config, false));
    }
    restore();
  }
  void visit(CodeBlock *node) override {
    ...
    m_config.brush = QBrush(QColor(249, 249, 249));
    m_instructionGroup.insertNewLineAt(0, new FillRectInstruction(m_config, false));
    restore();
  }
  ...
  void visit(UnorderedList *node) override {
    ...
    for (const auto &item : node->children()) {
      ...
      m_instructionGroup.appendItem(instruction);
      m_curX += 15;
      item->accept(this);
    }
  }
  ...
  [[nodiscard]] InstructionGroup renderRet() const { return m_instructionGroup; }
 ...
 private:
  ...
  void moveToNewLine() {
    ...
    m_instructionGroup.createNewLine();
  }
  // 辅助到绘制方法
 ...
 private:
  ...
  void drawText(const QRect &r, const PieceTableItem &item) {
    m_config.rect = r;
    m_instructionGroup.appendItem(new TextInstruction(item, m_config));
  }
  void drawText(const QRect &r, const String &str) {
    m_config.rect = r;
    m_instructionGroup.appendItem(new StaticTextInstruction(str, m_config));
  }

  void drawImage(const QRect &r, const String &imgPath) {
    m_config.rect = r;
    m_instructionGroup.appendItem(new StaticImageInstruction(imgPath, m_config));
  }
  void fillRect(const QRect &r, const QBrush &b) {
    m_config.rect = r;
    m_config.brush = b;
    m_instructionGroup.appendItem(new FillRectInstruction(m_config));
  }
  void drawLatex(const QRect &r, int size, const String &latex) {
    m_config.rect = r;
    m_config.font.setPixelSize(size);
    m_instructionGroup.appendItem(new LatexInstruction(latex, m_config));
  }
 ...
 private:
  ...
  DocPtr m_doc;
  InstructionGroup m_instructionGroup;
...
InstructionGroup Render::render(Node *node, DocPtr doc) {
  Q_ASSERT(node != nullptr);
  Q_ASSERT(doc != nullptr);
  static TexRenderGuard texRenderGuard;
  RenderPrivate render(doc);
  node->accept(&render);
