...
namespace md::parser {
struct Line {
  Line(const String& text, qsizetype offset, qsizetype length)
  ...
  [[nodiscard]] Line right(qsizetype n) const {
    return {text, offset + length - n, n};
  }
  const String& text;
  qsizetype offset;
  qsizetype length;
...
class LineParser {
 public:
  [[nodiscard]] virtual ParseResult parse(const TokenList& tokens,
                                          int startIndex) const = 0;
};
class BlockParser {
 public:
  [[nodiscard]] virtual ParseResult parse(const LineList& lines,
                                          int startIndex) const = 0;
...
class InlineCodeParser : public LineParser {
 public:
  [[nodiscard]] ParseResult parse(const TokenList& tokens,
 ...
 private:
  ...
  [[nodiscard]] ParseResult parseInlineCode(const TokenList& tokens,
...
class InlineLatexParser : public LineParser {
 ...
 private:
  ...
  [[nodiscard]] ParseResult _parse(const TokenList& tokens,
                                   int startIndex) const {
...
class SemanticTextParser : public LineParser {
 ...
 private:
  [[nodiscard]] static bool tryParseItalic(const TokenList& tokens,
                                           int startIndex) {
  ...
  [[nodiscard]] static bool tryParseBold(const TokenList& tokens,
                                         int startIndex) {
  ...
  [[nodiscard]] static bool tryParseItalicAndBold(const TokenList& tokens,
                                                  int startIndex) {
  ...
  [[nodiscard]] static bool tryParseStrickout(const TokenList& tokens,
                                              int startIndex) {
    if (startIndex + 5 > tokens.size()) return false;
    auto offset_list = {0, 1, 3, 4};
    auto text_offset = 2;
  ...
  static ParseResult _parseItalicAndBold(const TokenList& tokens,
                                         int startIndex) {
...
class LatexBlockParser : public BlockParser {
 public:
  [[nodiscard]] ParseResult parse(const LineList& lines,
                                  int startIndex) const override {
    if (startIndex < lines.size() && lines[startIndex].startsWith("$$")) {
 ...
 private:
  [[nodiscard]] ParseResult parseLatexBlock(const LineList& lines,
                                            int startIndex) const {
    int i = startIndex + 1;
    while (i < lines.size()) {
...
class ParagraphParser : public BlockParser {
 ...
 private:
  [[nodiscard]] ParseResult parseParagraph(const LineList& lines,
                                           int lineIndex) const {
...
class CodeBlockParser : public BlockParser {
 ...
 private:
  [[nodiscard]] ParseResult parseCodeBlock(const LineList& lines,
                                           int startIndex) const {
    if (startIndex >= lines.size()) return ParseResult::fail();
    int i = startIndex;
    auto line = lines[i];
...
class CheckboxListParser : public BlockParser {
 ...
 private:
  [[nodiscard]] ParseResult parseCheckboxList(const LineList& lines,
                                              int startIndex) const {
    static std::vector<LineParser*> parsers = {
        new LinkParser(),
        new InlineLatexParser(),
...
class UnorderedListParser : public BlockParser {
 ...
 private:
  [[nodiscard]] ParseResult parseUnorderedList(const LineList& lines,
                                               int startIndex) const {
    static std::vector<LineParser*> parsers = {
        new LinkParser(),
        new InlineLatexParser(),
...
class OrderedListParser : public BlockParser {
 ...
 private:
  [[nodiscard]] ParseResult parseOrderedList(const LineList& lines,
                                             int startIndex) const {
    static std::vector<LineParser*> parsers = {
        new LinkParser(),
        new InlineLatexParser(),
...
class QuoteBlockParser : public BlockParser {
 ...
 private:
  [[nodiscard]] ParseResult parseQuoteBlock(const LineList& lines,
                                            int startIndex) const {
    int i = startIndex;
...
DocPtr Parser::parse(const String& text) {
  ParserPrivate parser(text);
  DocPtr ret = std::make_shared<Document>(text);
  ret->m_root = parser.parse();
  return ret;
}
}  // namespace md::parser
