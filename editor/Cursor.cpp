...
void Cursor::moveX(int x) { m_x = x; }
void Cursor::moveY(int y) { m_y = y; }
void Cursor::updateHeight(int h) { m_h = h; }
...
QPoint Cursor::pos() const { return QPoint(m_x, m_y); }
QRect Cursor::region() const { return QRect(m_x, m_y, 1, m_h); }
...
void Cursor::setOffset(qsizetype offset) { m_coord.offset = offset; }
...
void Cursor::setRender(Render *render) { m_render = render; }
void Cursor::setNeedUpdateCoord(bool flag) { m_needUpdateCoord = flag; }
bool Cursor::needUpdateCoord() { return m_needUpdateCoord; }
CursorCoord Cursor::coord() { return m_coord; }
void Cursor::setCursorCoord(CursorCoord coord) { m_coord = coord; }
int Cursor::x() { return m_x; }
int Cursor::y() { return m_y; }
void Cursor::setEditorDocument(EditorDocument *doc) { m_doc = doc; }

void Cursor::insertText(QString text) { m_doc->insertText(text, *this); }
void Cursor::removeText(qsizetype length) { m_doc->removeText(length, *this); }
void Cursor::insertReturn() {}
